...
# (outcome) and a hospital ranking (num). 
...
# ranking specified in num. For example the function callrankall("heart attack",
...
rankhospDF <- function(state, outcome, dfOutcome, num = "best") {
        ## Read outcome data
...
rankall <- function(outcome, num = "best") {
        ...
        dfOutcome <- read.csv("outcome-of-care-measures.csv",
                             colClasses = "character")
        dfStates <- dfOutcome[,7]
        aStates<-tapply(dfStates,dfStates,max) #array of states
        ...
        ## rankhospital(state,outcome,num) can be used
        ## dfOutcome[,7] states list
        vHosps<-sapply(aStates,rankhospDF,outcome,dfOutcome,num)
        dfHospState<-as.data.frame(cbind(vHosps,aStates))
        colnames(dfHospState)<-c("hospital","state")
        dfHospState
}
